...
class GraphReplayBuffer(object):
    ...
    def __init__(self, 
        ...
        self.share_obs = np.zeros((self.episode_length + 1, 
                                    ...
                                    *share_obs_shape),
                                    dtype=np.float32)
        self.obs = np.zeros((self.episode_length + 1, 
                            self.n_rollout_threads, 
                            num_agents, 
                            *obs_shape), 
                            dtype=np.float32)
        # graph related stuff
        self.node_obs = np.zeros((self.episode_length + 1,
                                    self.n_rollout_threads,
                                    num_agents,
                                    *node_obs_shape),
                                    dtype=np.float32)
        ...
        self.agent_id = np.zeros((self.episode_length + 1,
                                    ...
                                    *agent_id_shape),
                                    dtype=np.int32)
        self.share_agent_id = np.zeros((self.episode_length + 1,
                                    self.n_rollout_threads,
                                    num_agents,
        ...
        self.rnn_states = np.zeros((self.episode_length + 1, 
                                    self.n_rollout_threads, 
                                    num_agents, 
                                    self.recurrent_N, 
                                    self.hidden_size),
        ...
        self.value_preds = np.zeros((self.episode_length + 1, 
                                    self.n_rollout_threads, 
                                    num_agents, 1), 
                                    dtype=np.float32)
        ...
        if act_space.__class__.__name__ == 'Discrete':
            self.available_actions = np.ones((self.episode_length + 1, 
                                            self.n_rollout_threads, 
                                            num_agents, 
                                            act_space.n),
                                            dtype=np.float32)
        ...
        self.actions = np.zeros((self.episode_length, 
                                self.n_rollout_threads, 
                                num_agents, 
                                act_shape), 
                                dtype=np.float32)
        self.action_log_probs = np.zeros((self.episode_length, 
                                        self.n_rollout_threads, 
                                        num_agents, 
                                        act_shape), 
                                        dtype=np.float32)
        self.rewards = np.zeros((self.episode_length, 
                                self.n_rollout_threads, 
                                num_agents, 1), 
                                dtype=np.float32)

        self.masks = np.ones((self.episode_length + 1, 
                            self.n_rollout_threads, 
                            num_agents, 1), 
                            dtype=np.float32)
        # print("self.masks.shape: ", self.masks.T)
        self.bad_masks = np.ones_like(self.masks)
        self.active_masks = np.ones_like(self.masks)
        # print("self.active_masks.shape: ",self.active_masks.T)
    ...
    def insert(self, 
        ...
        # print(self.step,"actions", actions.T)
        self.actions[self.step] = actions.copy()
        self.action_log_probs[self.step] = action_log_probs.copy()
        self.value_preds[self.step] = value_preds.copy()
        # print("buffer rewards",rewards.shape, self.rewards.shape)
        self.rewards[self.step] = rewards.copy()
        # print("Rewards", self.rewards.T)
        # print(self.step,"MASK : ", masks.T, "active_masks : ", active_masks.T)
        self.masks[self.step + 1] = masks.copy()
        # print(self.step,"buffer self.masks: ", self.masks.T)
        # print("buffer active_masks: ", active_masks.shape)
        if bad_masks is not None:
            self.bad_masks[self.step + 1] = bad_masks.copy()
        if active_masks is not None:
            # print(self.step,"active_masks",active_masks.T)
    ...
    def after_update(self) -> None:
        ...
        ## if self.masks[-1] is 0 then set new self.masks[0] to 0
        # print("self.masks[-1]",self.masks[-1])
        # print("self.masks[-1]",self.masks[-1].T)
        # self.masks[-1][self.masks[-1] == 0] = 1

        self.masks[0] = self.masks[-1].copy()
        # self.masks[0] = self.masks[-1].copy()
        # print("HAWWW---------------------------------------------------------------")
        # print(self.masks.T)
        # ## if self.active_masks[-1] is 0 then set new self.active_masks[0] to 0
        # if self.active_masks[-1] is 0:
        #     print("self.active_masks[-1]",self.active_masks[-1])
        #     print("self.active_masks[0]",self.active_masks[0])
        #     self.active_masks[0] = self.active_masks[-1].copy()
        # self.active_masks[-1][self.active_masks[-1] == 0] = 1

        self.active_masks[0] = self.active_masks[-1].copy()
        # print(self.active_masks.T)
    ...
    def compute_returns(self, 
                        next_value:arr, 
                        value_normalizer:Optional[PopArt]=None) -> None:
        """
        ...
        """
        # print("_use_proper_time_limits",self._use_proper_time_limits,"self._use_gae",self._use_gae)
        # print("self._use_popart or self._use_valuenorm",self._use_popart , self._use_valuenorm)
            ...
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        ...
                        self.returns[step] = gae + value_normalizer.denormalize(
                                                        self.value_preds[step])
                        # print(step,"self.returns",self.returns[step].T)
                    else:
                        delta = self.rewards[step] + self.gamma * \
                            self.value_preds[step + 1] * self.masks[step + 1] - \
                            self.value_preds[step]
    ...
    def feed_forward_generator(self, 
                            ...
                            mini_batch_size:Optional[int]=None) -> Generator[
                            Tuple[arr, arr, arr, arr, arr, arr, arr, arr,
        ...
        rnn_states_critic = self.rnn_states_critic[:-1].reshape(-1, 
                                            *self.rnn_states_critic.shape[3:])
        actions = self.actions.reshape(-1, self.actions.shape[-1])
        if self.available_actions is not None:
            available_actions = self.available_actions[:-1].reshape(-1, 
        ...
        for indices in sampler:
            ...
            yield share_obs_batch, obs_batch, node_obs_batch, adj_batch, \
                agent_id_batch, share_agent_id_batch, rnn_states_batch, \
    ...
    def naive_recurrent_generator(self, 
                                advantages:arr, 
                                num_mini_batch:int) -> Generator[
                                Tuple[arr, arr, arr, arr, arr, arr, arr, arr,
                                arr, arr, arr, arr, arr, arr, arr, arr], 
                                    None, None]:
        """
        ...
        assert n_rollout_threads * num_agents >= num_mini_batch, (
            ...
            "to be greater than or equal to the number of "
            "PPO mini batches ({}).".format(n_rollout_threads, 
            num_agents, num_mini_batch))
        num_envs_per_batch = batch_size // num_mini_batch
        perm = torch.randperm(batch_size).numpy()

        share_obs = self.share_obs.reshape(-1, batch_size, 
                                        *self.share_obs.shape[3:])
        obs = self.obs.reshape(-1, batch_size, 
                                    *self.obs.shape[3:])
        node_obs = self.node_obs.reshape(-1, batch_size, 
                                        *self.node_obs.shape[3:])
        adj = self.adj.reshape(-1, batch_size, 
                                        *self.adj.shape[3:])
        agent_id = self.agent_id.reshape(-1, batch_size, 
                                        *self.agent_id.shape[3:])
        share_agent_id = self.share_agent_id.reshape(-1, batch_size, 
                                        *self.share_agent_id.shape[3:])
        rnn_states = self.rnn_states.reshape(-1, batch_size, 
                                        *self.rnn_states.shape[3:])
        rnn_states_critic = self.rnn_states_critic.reshape(-1, batch_size, 
                                        *self.rnn_states_critic.shape[3:])
        actions = self.actions.reshape(-1, batch_size, self.actions.shape[-1])
        if self.available_actions is not None:
            available_actions = self.available_actions.reshape(-1, batch_size, 
                                            self.available_actions.shape[-1])
        ...
        action_log_probs = self.action_log_probs.reshape(-1, batch_size, 
                                                self.action_log_probs.shape[-1])
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            rnn_states_batch = np.stack(rnn_states_batch).reshape(N, 
                                                    *self.rnn_states.shape[3:])
            rnn_states_critic_batch = np.stack(
                                        rnn_states_critic_batch).reshape(N, 
                                        *self.rnn_states_critic.shape[3:])
    ...
    def recurrent_generator(self, 
                            advantages:arr, 
                            num_mini_batch:int, 
                            data_chunk_length:int) -> Generator[
                            Tuple[arr, arr, arr, arr, arr, arr, arr, arr,
                            arr, arr, arr, arr, arr, arr, arr, arr], 
                                None, None]:
        """
        ...
        # print("1share_obs.shape",self.share_obs.shape, self.share_obs)
        if len(self.share_obs.shape) > 4:
            share_obs = self.share_obs[:-1].transpose(1, 2, 0, 
                        3, 4, 5).reshape(-1, *self.share_obs.shape[3:])
            obs = self.obs[:-1].transpose(1, 2, 0, 
                        3, 4, 5).reshape(-1, *self.obs.shape[3:])
        else:
            share_obs = _cast(self.share_obs[:-1])
            obs = _cast(self.obs[:-1])
        # print("2share_obs.shape",share_obs.shape, share_obs)
        node_obs = self.node_obs[:-1].transpose(1, 2, 0, 3, 4).reshape(-1, *self.node_obs.shape[3:])
        ...
        # print("masks.shape: ", masks.shape, masks.T)
        active_masks = _cast(self.active_masks[:-1])
        # print("ACT.active_masks: ", self.active_masks.T)
        # rnn_states = _cast(self.rnn_states[:-1])
        # rnn_states_critic = _cast(self.rnn_states_critic[:-1])
        rnn_states = self.rnn_states[:-1].transpose(1, 2, 0, 
                        3, 4).reshape(-1, *self.rnn_states.shape[3:])
        rnn_states_critic = self.rnn_states_critic[:-1].transpose(1, 2, 0, 
                            3, 4).reshape(-1, *self.rnn_states_critic.shape[3:])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                # print("ind", ind, "data_chunk_length", data_chunk_length)
                ...
                # print("ind", ind, "data_chunk_length", data_chunk_length)
                # print(active_masks[ind:ind + data_chunk_length].T)
                active_masks_batch.append(active_masks[ind:ind + 
                                                        data_chunk_length])
                # print("active_batch", active_masks_batch[-1].T)
                old_action_log_probs_batch.append(action_log_probs[ind:ind + 
                                                            data_chunk_length])
            ...
            # print("share_obs_batch",len(share_obs_batch), share_obs_batch)
            ...
            # print("BATCH",active_masks_batch.T)
            yield share_obs_batch, obs_batch, node_obs_batch, adj_batch, \
                agent_id_batch, share_agent_id_batch, rnn_states_batch, \
                rnn_states_critic_batch, actions_batch, value_preds_batch, \
                return_batch, masks_batch, active_masks_batch, \
